...
unit    class Hypervisor::IBM::POWER::HMC::REST::ManagedSystems::ManagedSystem::LogicalPartitions::LogicalPartition::PartitionProcessorConfiguration::CurrentSharedProcessorConfiguration:api<1>:auth<Mark Devine (mark@markdevine.com)>
...
method init () {
    ...
    $!CurrentUncappedWeight             = self.etl-text(:TAG<CurrentUncappedWeight>,            :$!xml, :optional)  if self.attribute-is-accessed(self.^name, 'CurrentUncappedWeight');
    ...
    $!RuntimeUncappedWeight             = self.etl-text(:TAG<RuntimeUncappedWeight>,            :$!xml, :optional)  if self.attribute-is-accessed(self.^name, 'RuntimeUncappedWeight');
    $!xml                               = Nil;
    $!initialized                       = True;
    self;
}
